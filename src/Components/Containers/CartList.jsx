...
export default function CartList() {
  ...
  const fetchCartList = () => {
    ...
    axios.post(`${API_URL.CUSTOMER_CART_ITEMS}/${AuthID}`).then((response) => {
      ...
      console.log('localStorage', localStorage.getItem("CartTestList"));
    })
  }
  ...
  useEffect(() => {
    ...
    const testListFromCart = JSON.parse(localStorage.getItem("CartTestList"));
    console.log('testListFromCart', testListFromCart);

    if (testListFromCart) {
      let CalculateTotalTestPrice = 0;
      testListFromCart.forEach((item) => {
      ...
      console.log('CalculateTotalTestPrice', CalculateTotalTestPrice);
    }
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {CartTable.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="text-right">
                            {item.IsPackage === "Yes" ? (
                              <span className="px-2 py-1 badge badge-success">
                                <i className="fa fa-flask" aria-hidden="true"></i> Package
                              </span>
                            ) : (
                              <span className="px-2 py-1 badge badge-primary">
                                <i className="fa fa-flask" aria-hidden="true"></i> Test
                              </span>
                            )}
                          </td>
                      ...
                      <tr>
                        ...
                        <td className="text-right">
                          <b style={{ fontSize: '16px' }}>₹ {testTotal}</b>
                        </td>
                        <td>
                          <div className="case m-0 text-center">
                            <p>
                              <Link to="/checkout" onClick={handleBeginCheckout}>
                                Checkout
                              </Link>
                            </p>
                          </div>
                ...
                <div className="col-12">
                      ...
                      <ul className="list-group">
                        {CartTable.map((item, index) => (
                          <li className="list-group-item" key={index}>
                            <h6>
                              {item.IsPackage === "Yes" ? (
                                <small className="px-2 py-1 badge badge-success">
                                  <i className="fa fa-flask" aria-hidden="true"></i> Package
                                </small>
                              ) : (
                                <small className="px-2 py-1 badge badge-primary">
                                  <i className="fa fa-flask" aria-hidden="true"></i> Test
                                </small>
                              )}
                            </h6>
                            ...
                            <div className="d-flex w-100 justify-content-between">
                              ...
                              <button
                                className="btn-light border-0 outline-0 rounded pt-2"
                  ...
                  <div className="availab-lity ca-rtloc mt-3">
                    ...
                    <div className="case text-right">
                      <p>
                        <Link to="/" onClick={handleBeginCheckout}>
                          Checkout
                        </Link>
                      </p>
                    </div>
